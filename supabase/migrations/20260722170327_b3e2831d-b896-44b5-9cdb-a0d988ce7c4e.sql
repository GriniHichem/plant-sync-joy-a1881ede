...
-- Product code generation config
ALTER TABLE public.reception_products
  ADD COLUMN IF NOT EXISTS code_prefix text,
  ADD COLUMN IF NOT EXISTS code_digits smallint;

DO $$ BEGIN
  ALTER TABLE public.reception_products
    ADD CONSTRAINT reception_products_digits_ck CHECK (code_digits IS NULL OR (code_digits BETWEEN 1 AND 12));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Audit: raw number entered by the agent
ALTER TABLE public.reception_weighings
  ADD COLUMN IF NOT EXISTS code_saisi text;

-- Recreate global view including new product fields
...
SELECT t.id, t.numero, t.date_ticket, t.heure_debut, t.heure_fin,
       ...
       p.code_prefix AS produit_code_prefix, p.code_digits AS produit_code_digits,
       s.id AS supplier_id, s.nom AS fournisseur, s.code AS fournisseur_code, s.region, s.wilaya,
       w.id AS weighing_id, w.code_pesee, w.code_saisi,
       w.poids_brut_kg, w.poids_abattement_kg, w.poids_net_kg, w.weighed_at,
       (CASE
